...
c
c  PURPOSE  sorts a sequence of integers into ascending order by ordering
c           an index vector
c
c  DESCRIPTION
c
c  ARGUMENTS
c
c     arr	integer vector of length at least n which contains the raw
...
5       indx(l)=indx(j)
        indx(j)=indxt
        jstack=jstack+2
c        if(jstack.gt.NSTACK)pause 'NSTACK too small in isorti'
