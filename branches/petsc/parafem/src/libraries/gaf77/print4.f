...
c
c  PURPOSE  prints four values according to a C style format
c
c  This routine takes as input a format, which is a character string
c  containing C style format descriptors, along with four additional
c  arguments and prints them to the given unit. For example, say you wish
c  to print the values of the variables 'i', `d', 'j', and 'f' to unit 6
c  so that the output appears as
c
c	d(3) = 16.49, f(12) = -0.03
c
c  (assuming i = 3, d = 16.49, j = 12, and f = -0.03), then this routine
c  would be called as follows;
c
c	call print4( 6, 'd(%i) = %f, f(%i) = %f%n', i, d, j, f )
c
...
c
c  NOTES:
c   1)	if more than 4 occurrences of the formats %f, %e or %i appear in
