...
c
c-------------------------------------------------------------------------
      real*8 function dfdist(x,v1,v2)
...
c					check for limiting cases in v1,v2
      if( v1+v2 .le. big ) then
         dfdist = one - beta((v2/(v2+v1*x)), half*v2, half*v1)
      elseif( v2 .gt. (big + s*v1) ) then
         dfdist = gmdst( v1*x, half*v1, two )
      elseif( v1 .gt. (big + s*v2) ) then
         dfdist = one - gmdst( v2/x, half*v2, two )
      else
         a1 = v2/(v2-two)
         a2 = sqrt( two*(v1+v2-two)/(v1*(v2-four)) )
         dfdist = phi( (x - a1)/(a1*a2) )
      endif
