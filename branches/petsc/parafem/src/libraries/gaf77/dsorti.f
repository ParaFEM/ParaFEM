...
c
c  PURPOSE  sorts a sequence of real*8 numbers into ascending order by ordering
c           an index vector
c
c  DESCRIPTION
c
...
5       indx(l)=indx(j)
        indx(j)=indxt
        jstack=jstack+2
c        if(jstack.gt.NSTACK)pause 'NSTACK too small in dsorti'
