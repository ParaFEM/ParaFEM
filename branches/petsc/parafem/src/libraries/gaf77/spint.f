...
c-----------------------------------------------------------------------------
      real function spint( x, f, b, n, LCLAMP, clamp1, clamp2, ierr)
      dimension x(*), f(*), b(*)
      logical LCLAMP
      data zero/0.0/, half/0.50/, onept5/1.50/, two/2.0/
      data three/3.0/, four/4.0/, forpt5/4.50/
...
c						compute integral
      ...
      spint = b(3*n-2)*(f(1) + b(3*n-2)*(b(1) + b(3*n-2)*(b(n)
     >       + b(3*n-2)*b(2*n-1))))
      do 70 i = 2, n-1
         spint = spint + b(3*n-3+i)*(f(i)+b(3*n-3+i)*(b(i)
