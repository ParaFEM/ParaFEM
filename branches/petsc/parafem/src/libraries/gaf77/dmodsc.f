...
c  equal to 2.0e-20 may be represented as zero in double precision.
c
c  The routine checks for the occurrence of a zero slope. If encountered,
c  the `convergence failed' flag is set (nit < 0) and dmodsc returns the
...
c
c-----------------------------------------------------------------------------
      real*8 function dmodsc( G, DG, xa, xb, tol, maxit, nit )
...
c					new estimate of root
         dmodsc = x1 - u1*(x0-x1)/den
c					relative error estimate
         if( dmodsc .eq. zero ) then
            e = abs( (dmodsc - x1)/deps )
         else
            e = abs( (dmodsc - x1)/dmodsc )
         endif
