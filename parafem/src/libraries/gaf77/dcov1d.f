...
c  Written by Gordon A. Fenton, TUNS, Dec 11, 1997
c  Latest Update: Dec 11, 1997
c
c  PURPOSE  computes the sample mean, variance, and covariance function from
c           a discrete sequence of observations taken from a 1-D
c           random process.
c
c  DESCRIPTION
c  This routine computes the sample mean, variance, and covariance function
...
c  sequence and average the mean, variance, and covariance estimates over
c  the ensemble. In the event that the realizations consist of different
c  numbers of observations, the trailing covariances (from the maximum lag
...
c		the estimated covariance function. If nit > 1, then
...
c		is false, in which case, covariances from lag zero up to
...
c		has been called. When nen = nit, then the average covariance
...
c  REVISION HISTORY:
c---------------------------------------------------------------------------
      subroutine dcov1d(z,n,ns,zm,zmax,zmin,zv,zvmax,zvmin,
...
c					estimate the covariance function
