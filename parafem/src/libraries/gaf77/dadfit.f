...
c  ARGUMENTS
c
c	x	real*8 vector of length at least n containing the original
c		data (or log-data if the distribution type is lognormal,
c		using ntype = 1). (input)
c
c	z	real*8 vector of length at least n used as workspace.
c
...
c
c   alpha	real*8 significance value at which to conduct the test (see
...
c		test. (input)
c
c      p1	real*8 value containing the first known (or estimated)
...
c		(input)
c
c      p2	real*8 value containing the second known (or estimated)
c		parameter of the null distribution. In the case of the
c		0) distribution with all parameters known, this is not used
...
c-------------------------------------------------------------------------
      real*8 function dadfit(x,z,n,ntype,reject,alpha,p1,p2,ierr)
      implicit real*8 (a-h,o-z)
      real*8 x(*), z(*), alpha, p1, p2
      real*8 adcrit(4,4), tol
      real*8 pt01, pt0175, pt025, pt0375, pt05, pt075, pt1
      logical reject
      data zero/0.d0/, small/0.001d0/, half/0.5d0/
      data one/1.d0/, four/4.d0/, twnty5/25.d0/
      data pt01/0.01d0/, pt0175/0.0175d0/, pt025/0.025d0/
      data pt0375/0.0375d0/, pt05/0.05d0/, pt075/0.075d0/
      data pt1/0.1d0/, pt2/0.2d0/, pt6/0.6d0/
      data tol/0.001d0/
c					Anderson-Darling Critical Values
c           Alpha = 0.10  0.05   0.025  0.01

      data adcrit/1.933d0, 2.492d0, 3.070d0, 3.857d0,	! all par known
     >            0.632d0, 0.751d0, 0.870d0, 1.029d0,	! normal
     >            1.070d0, 1.326d0, 1.587d0, 1.943d0,	! exponential
     >            0.637d0, 0.757d0, 0.877d0, 1.038d0/	! Weibull
...
c					decode significance level
      if( dabs(alpha-pt1) .lt. tol ) then
         i = 1
      elseif( dabs(alpha-pt05)  .lt. tol ) then
         i = 2
      elseif( dabs(alpha-pt025) .lt. tol ) then
         i = 3
      elseif( dabs(alpha-pt01)  .lt. tol ) then
         i = 4
      else
