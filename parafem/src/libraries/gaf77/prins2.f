...
c
c  PURPOSE  prints two values according to a C style format to an internal
c           character string.
c
...
c  to print the values of the variables 'i', and `d'
c  to the string sub so that the string appears as
c
c	d(3) = 16.49
c
c  (assuming i = 3, and d = 16.49), then this routine would be called
c  as follows;
c
c	call prins2( sub,
c      >    'd(%i) = %f', i, d )
...
c-------------------------------------------------------------------------
      subroutine prins2( s, fmt, val1, val2 )
      parameter (MP = 2)
      character*(*) fmt, s
