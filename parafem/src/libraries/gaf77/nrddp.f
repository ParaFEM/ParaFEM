c  *******************************************************************
c  *                                                                 *
c  *                    integer function nrddp                       *
c  *                                                                 *
c  *******************************************************************
c  Double Precision Version 2.4
...
c
c       str = '2.2d+03  3.2  45  5.r2  6'
c
c  with the reference `n = nrddp( str, v, 20 )' would result in n = -4
c  with v(1) = 2200.0, v(2) = 3.2, v(3) = 45.0. Note that the string (see
c  discussion below)
c
c       str = '2.2d+03 3.2-4.2,0.2 45 5.r2 6'
c
...
c  2.4	use of integer f(3) only allows about 10 digits in some cases.
c	Made f(3) real*8 to allow a very large number of specified
c	digits. (Nov 26/04)
c--------------------------------------------------------------------------
      integer function nrddp(str,v,MX)
      real*8 v(*), s, fr, f1, f2, f3, dble
      real*8 f(3)
      character*(*) str
     ...
     >   .or. c .eq. 'd' .or. c .eq. 'D' ) then
      ...
      elseif( (c .eq. spc) .or. (c .eq. tab) .or. (i .ge. ls) ) then
         if( lseq ) then
            ...
            if( f3 .eq. 0.d0 ) go to 999
            n  = idint( 1.000000000001d0*(f2 - f1)/f3 )
            if( n .lt. 0 ) go to 999
            do 30 L = 0, n
