...
c
c  PURPOSE  prints seven values according to a C style format
c
c  This routine takes as input a unit number, a format, which is a character
c  string containing C style format descriptors, along with seven additional
c  arguments and prints them to the given unit. For example, say you wish
c  to print the values of the variables 'i', `d', 'j',  'f', 'r', 'nsim',
c  and 'ierr' to unit 6 so that the output appears as
c
c	d(3) = 16.49, f(12) = -0.03 + 8.2 (6'th realization, ierr = 0)
c
c  (assuming i = 3, d = 16.49, j = 12, f = -0.03, r = 8.2, nsim = 6,
c  and ierr = 0), then this routine would be called as follows;
c
c	call print7( 6,
c      >    'd(%i) = %f, f(%i) = %f + %f (%i''th realization, ierr = %i)%n',
c      >             i, d, j, f, r, nsim, ierr )
c
...
c
c  NOTES:
c   1)	if more than 7 occurrences of the formats %f, %e or %i appear in
...
c  1.13	modified above writeup to reflect 7 print objects (Jul 14/00)
c  1.2	modified above writeup to reflect new formats and escapes (Oct 4/01)
c-------------------------------------------------------------------------
      subroutine print7(k,fmt,val1,val2,val3,val4,val5,val6,val7 )
      parameter (MP = 7)
      character fmt*(*)
      real val1, val2, val3, val4, val5, val6, val7, tmp(MP)
