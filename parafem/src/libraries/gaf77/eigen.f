...
c  *                                                                  *
c  ********************************************************************
c  Single Precision Version 2.01
...
c-----------------------------------------------------------------------------
      subroutine eigen( A, ia, d, e, n, LQ, ORDER, ierr )
      parameter (MAXIT = 30)
      dimension A(ia,*), d(*), e(*)
      logical LQ, ORDER
      data zero/0.0/, one/1.0/, two/2.0/
c					external functions used
c     dabs(x) = abs(x)			(to convert to real*8)
c     dsqrt(x) = sqrt(x)		(not used now)
c     dsign(x1,x2) = sign(x1,x2)
