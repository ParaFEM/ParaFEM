...
c---------------------------------------------------------------------------
      subroutine lnvmp( A, ALU, AI, ia, LPP, indx, b, r, z, n, ierr )
...
c						yes, use partial pivoting
  ...
  20     continue
         do 30 j = 1, n
            b(j) = one
            call psolv( ALU, ia, n, indx, b, AI(1,j) )
            call imprv( A, ALU, ia, n, indx, AI(1,j), b, r, z, 1 )
            b(j) = zero
  30     continue
      else
c						no, use naive Gauss Elim.
  ...
  40     continue
         do 50 j = 1, n
            b(j) = one
            call nsolv( ALU, ia, n, AI(1,j) )
            call nmprv( A, ALU, ia, n, AI(1,j), b, r, 1 )
            b(j) = zero
  50     continue
      endif
