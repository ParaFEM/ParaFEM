...
c
c  PURPOSE  reads four real/integer values according to a C style `sscanf'
...
c  appear in the fmt string than arguments to this routine (4), then
...
c	name from scans4 to nscan4. (Jul 13/04)
c-------------------------------------------------------------------------
      integer function nscan4( s, fmt, val1, val2, val3, val4 )
      parameter (MP = 4)
      character*(*) fmt, s
      real val1, val2, val3, val4, tmp(MP)

      nscan4 = nscanv( s, fmt, tmp, MP )
