...
c
c  PURPOSE  prints seven values according to a C style format to an internal
c           character string.
c
c  This routine takes as input a unit number, a format, which is a character
c  string containing C style format descriptors, and seven additional
...
c  and 'ierr' to the string sub so that the string appears as
c
c	d(3) = 16.49, f(12) = -0.03 + 8.2 (6'th realization, ierr = 0)
c
c  (assuming i = 3, d = 16.49, j = 12, f = -0.03, r = 8.2, nsim = 6, and
c  ierr = 0), then this routine would be called as follows;
c
c	call prins7( sub,
c      >    'd(%i) = %f, f(%i) = %f + %f (%i''th realization, ierr = %i)',
...
c  1.12	modified above writeup to reflect 7 print objects (Jul 14/00)
c  1.2	modified above writeup to reflect new formats and escapes (Oct 4/01)
c-------------------------------------------------------------------------
      subroutine prins7( s,fmt,val1,val2,val3,val4,val5,val6,val7 )
      parameter (MP = 7)
      character*(*) fmt, s
