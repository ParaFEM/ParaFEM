...
c
c  PURPOSE  reads three real/integer values according to a C style `sscanf'
...
c  problems encountered with nscan2 and similar statements can be made about
...
c  appear in the fmt string than arguments to this routine (3), then
...
c	name from scans3 to nscan3. (Jul 13/04)
c-------------------------------------------------------------------------
      integer function nscan3( s, fmt, val1, val2, val3 )
      parameter (MP = 3)
      character*(*) fmt, s
      real val1, val2, val3, tmp(MP)

      nscan3 = nscanv( s, fmt, tmp, MP )
