...
c
c  PURPOSE  prints five values according to a C style format
c
...
c  to print the values of the variables 'i', `d', 'j',  'f' and 'ierr'
c  to unit 6 so that the output appears as
c
c	d(3) = 16.49, f(12) = -0.03 (ierr = 0)
c
c  (assuming i = 3, d = 16.49, j = 12, f = -0.03, and ierr = 0), then this
c  routine would be called as follows;
c
c	call print5( 6, 'd(%i) = %f, f(%i) = %f (ierr = %i)%n',
c      >             i, d, j, f, ierr )
c
...
c
c  NOTES:
c   1)	if more than 5 occurrences of the formats %f, %e or %i appear in
...
c-------------------------------------------------------------------------
      subroutine print5( k, fmt, val1, val2, val3, val4, val5 )
      parameter (MP = 5)
      character*(*) fmt
