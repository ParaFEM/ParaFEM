...
c  Written by Gordon A. Fenton, Dalhousie University, Sep 30, 2001
c  Latest Update: Sep 30, 2001
c
c  PURPOSE  prints eight values according to a C style format to an internal
c           character string.
c
...
c  to print the values of the variables nx1, nx2, nx3, nye, xl, yl, w1, and
c  w2 to the string sub so that the string appears as
c
...
c
c        call prins8(
c        'n_e = (%i + %i + %i x %i), D = (%f x %f), w_1 = %f, w_2 = %f',
...
c  REVISION HISTORY
c-------------------------------------------------------------------------
      subroutine prins8( s,fmt,val1,val2,val3,val4,val5,val6,val7,val8 )
      parameter (MP = 8)
      character*(*) fmt, s
