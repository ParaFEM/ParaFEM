...
c  *                                                                *
c  ******************************************************************
c  Single Precision Version 1.02
...
c
c      A   real vector of length N = 2**m. On input A contains the real part
c          of the observed series. On output A contains the fourier
c          coefficients A(k) corresponding to the frequencies 2*pi*k/(N*s)
c          where s is the time (or space) increment.
c
...
c          coefficients B(k).
c
c      m   the length of the Fourier sequence considered is N = 2**m.
c
...
c  1.02	corrected documentation above to read A(k)*cos - B(k)*sin (Apr 26/06)
c------------------------------------------------------------------------------
      Subroutine fft1d( A, B, m, INV )
      logical INV
      real*4 A(*), B(*), sr, si, ninv
      real*8 wr, wi, sygn, pi, pibyk, tr, ti, qr, qi, zero, one
      real*8 dcos, dsin
