...
c
c  PURPOSE  reads six real/integer values according to a C style `sscanf'
...
c  appear in the fmt string than arguments to this routine (6), then
...
c
c    val6	the sixth output argument. (output)
c
c  REVISION HISTORY:
c  1.1	converted to a function returning number of items read, changed
c	name from scans6 to nscan6. (Jul 13/04)
c-------------------------------------------------------------------------
      integer function nscan6( s, fmt, val1, val2, val3,
     >                         val4, val5, val6 )
      parameter (MP = 6)
      character*(*) fmt, s
      real val1, val2, val3, val4, val5, val6, tmp(MP)

      nscan6 = nscanv( s, fmt, tmp, MP )
