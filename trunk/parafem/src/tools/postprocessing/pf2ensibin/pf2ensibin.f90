...
!------------------------------------------------------------------------------ 
  ...
  INTEGER,ALLOCATABLE   :: sense(:),node(:),tempres_int(:)
...
!  CALL READ_NODES(fname,nn,nn_start,numpe,g_coord)
  ...
  tempres = zero
  IF(numpe==1) WRITE(13) int(nn,kind=c_int)
  DO i=1,ndim; tempres=zero
    DO j=1,nodes_pp
      tempres(j)=g_coord(i,j)
    END DO
    CALL dismsh_ensi_pb2(13,1,nodes_pp,npes,numpe,nodof,tempres)
  END DO
  DEALLOCATE(tempres)
  ...
  END IF
  ALLOCATE(tempres_int(nels_pp*nod))
  tempres_int = 0  
  DO i = 1,nels_pp
    tempres_int((i-1)*4+1) = g_num_pp(1,i)
    tempres_int((i-1)*4+2) = g_num_pp(3,i)
    tempres_int((i-1)*4+3) = g_num_pp(2,i)
    tempres_int((i-1)*4+4) = g_num_pp(4,i)
!    WRITE(13) int(g_num_pp(1,i),kind=c_int),int(g_num_pp(3,i),kind=c_int), &
!              int(g_num_pp(2,i),kind=c_int),int(g_num_pp(4,i),kind=c_int)
  END DO
  CALL dismsh_ensi_pb2_int(13,1,nels_pp,npes,numpe,nod,tempres_int)
  DEALLOCATE(tempres_int)
  IF(numpe==1) CLOSE (13)
  
!----------------------------------------------------------------------------
! 11b. Write old geometry file
!----------------------------------------------------------------------------
  
  IF(numpe==1)THEN
    fname   = job_name(1:INDEX(job_name, " ")-1)//".bin.ensi.geo_old"
    OPEN(24,FILE=fname,STATUS="REPLACE",FORM="UNFORMATTED",                   &
                  ACTION="WRITE", ACCESS="STREAM")
    
    cbuffer = "C Binary"                     ; WRITE(24) cbuffer
    cbuffer = "Problem name: "//job_name(1:INDEX(job_name, " ")-1) ; WRITE(24) cbuffer
    cbuffer = "Geometry files"               ; WRITE(24) cbuffer
    cbuffer = "node id off"                  ; WRITE(24) cbuffer
    cbuffer = "element id off"               ; WRITE(24) cbuffer
!  cbuffer = "node id given"                  ; WRITE(13) cbuffer
!  cbuffer = "element id given"               ; WRITE(13) cbuffer
...
!  END IF
    IF(ndim==3) THEN
       cbuffer = "Volume"                    ; WRITE(24) cbuffer
!     cbuffer = "Volume Mesh"                    ; WRITE(13) cbuffer
    END IF
...
!  IF(numpe==1) PRINT *, "g_coord"  
    WRITE(24) int(nn,kind=c_int)
    DO j=1,ndim
      DO i=1,nn
!        IF(numpe==1) PRINT *, g_coord(j,i)
        WRITE(24) real(g_coord(j,i),kind=c_float)
      END DO
    END DO
...
!      SELECT CASE(nod)
!        CASE(4)
            cbuffer = "tetra4" ; WRITE(24) cbuffer
            WRITE(24) int(nels,kind=c_int)
            DO i = 1,nels
!              WRITE(24) int(g_num_pp(1,i),kind=c_int),int(g_num_pp(3,i),kind=c_int), &
!                        int(g_num_pp(2,i),kind=c_int),int(g_num_pp(4,i),kind=c_int)
              WRITE(24) int(g_num_pp(1,i),kind=c_int)
              WRITE(24) int(g_num_pp(3,i),kind=c_int)
              WRITE(24) int(g_num_pp(2,i),kind=c_int)
              WRITE(24) int(g_num_pp(4,i),kind=c_int)
